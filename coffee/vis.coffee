...
Bubbles = () ->
  ...
  tick = (e) ->
    ...
    label
      .style("left", (d) -> ((margin.left + d.x) - d.dx / 1.99) + "px")
	  
      .style("top", (d) -> ((margin.top + d.y) - d.dy / 2.5) + "px")
  ...
  updateLabels = () ->
    ...
    #  styling divs
    label
      .style("font-size", (d) -> Math.max(8, rScale(rValue(d) / 2.5)) + "px")
  ...
  updateActive = (id) ->
   ...
   # if id.length > 0
    #  d3.select("#status").html("<h3><span class=\"active\">#{id}</span> is in current tweets</h3>")
...
texts = [
  ...
  {key:"tweets",file:"input.csv",name:"Current Tweets"}
